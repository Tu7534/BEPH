"""
================================================================================
脚本名称: train.py (MorphGAT - 泛癌微环境 233 维专版 - 双重注意力满血版)
级别: 工业级标准 (Production-Ready)
功能描述:
【泛癌终极版】：
1. 动态门控残差融合 (Gating)，解决特征量级压制。
2. 自适应困难负样本挖掘 (Adaptive 10% Quantile)。
3. 空间多正样本 (Multi-positive InfoNCE)，极大平滑组织空间拓扑边界。
4. 安全边缘属性缩放，避免 log(0) 梯度爆炸。
5. AdamW + Warmup + CosineAnnealingLR 工业级炼丹策略。
6. ✨ [NEW] 植入 PathwayFeatureAttention (通道特征注意力机制)，实现特征与空间双重解耦。
================================================================================
"""
import os
...
class PathwayFeatureAttention(nn.Module):
    """
    ✨ [NEW] 轻量级的特征注意力模块 (SENet 风格)
    在图卷积降维之前，为 233 维生物学通路计算自适应权重，压制噪声特征
    """
    def __init__(self, in_features=233, reduction_ratio=2):
        super().__init__()
        ...
        self.attention_mlp = nn.Sequential(
            ...
            nn.Sigmoid() # 输出 (0~1) 的权重概率
        )

    def forward(self, x):
        # x shape: [n_spots, 233]
        pathway_weights = self.attention_mlp(x) 
        # 将学习到的特征权重与原始特征进行逐元素相乘，实现自适应过滤
        x_weighted = x * pathway_weights 
        return x_weighted, pathway_weights
...
class GCLModel_Morph(nn.Module):
    def __init__(self, in_channels=233, hidden_channels=128, out_channels=64, n_clusters=20):
        super().__init__()
        self.heads1 = 8 
        ...
        # 用于保存前向传播时计算出的通路权重，方便下游任务调用
        self.saved_pathway_weights = None

    def forward(self, x, edge_index, edge_attr):
        # ✨ [NEW] 第一步：在降维和建图之前，立刻应用特征注意力机制
        # x_weighted 形状: [N_spots, 233], self.saved_pathway_weights 形状: [N_spots, 233]
        x_weighted, self.saved_pathway_weights = self.feature_attn(x)
        
        # 后续操作全部基于被“降噪过滤”后的 x_weighted 进行
...
def train(args):
    ...
    logger.info(f"🚀 初始化泛癌训练任务 (Multi-Positive + Feature Attention) | Device: cuda")
    ...
    optimizer_pre = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=1e-4)
    warmup_epochs = 15
    sched_warmup = LinearLR(optimizer_pre, start_factor=0.1, total_iters=warmup_epochs)
    sched_cosine = CosineAnnealingLR(optimizer_pre, T_max=args.pretrain_epochs - warmup_epochs)
    ...
    for epoch in range(args.pretrain_epochs):
        ...
        for b_orig, b_corr in train_loader:
            ...
            with torch.cuda.amp.autocast(enabled=(DEVICE.type == 'cuda')):
                ...
                loss_cl = spatial_contrastive_loss(z1, z2, b_orig.edge_index, b_orig.x, batch_info, temperature=args.temp, hard_weight=args.hard_weight)
        ...
        if epoch % 5 == 0: 
            current_lr = optimizer_pre.param_groups[0]['lr']
            logger.info(f"Pre-train Epoch {epoch:03d} | LR: {current_lr:.6f} | Loss: {avg_loss:.4f}")
    ...
    for epoch in range(args.epochs):
        ...
        # ... 后面的 val_loader 代码依此类推，也加上 >= dec_warmup_epochs 的判断即可 ...
        ...
        with torch.no_grad():
            for b_orig, b_corr in val_loader:
                ...
                batch_info = b_orig.batch if hasattr(b_orig, 'batch') else torch.zeros(b_orig.x.size(0), dtype=torch.long, device=DEVICE)
                v_loss_cl = spatial_contrastive_loss(z1, z2, b_orig.edge_index, b_orig.x, batch_info, temperature=args.temp, hard_weight=args.hard_weight)
        ...
        if epoch % 5 == 0:
            logger.info(f"Fine-tune Epoch {epoch:03d} | Train Loss: {avg_train_loss:.4f} | Val Loss: {avg_val_loss:.4f}")
    ...
    plt.title('Training Loss Pipeline (Multi-Positive & Dual Attention)')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lambda_dec", type=float, default=0.5)
