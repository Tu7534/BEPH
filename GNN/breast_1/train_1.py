"""
================================================================================
脚本名称: train.py (MorphGAT - 泛癌微环境 233 维专版 - 满血工业级)
级别: 工业级标准 (Production-Ready)
功能描述:
【泛癌终极版】：
1. 动态门控残差融合 (Gating)，解决特征量级压制。
2. 自适应困难负样本挖掘 (Adaptive 10% Quantile)。
3. [NEW] 空间多正样本 (Multi-positive InfoNCE)，极大平滑组织空间拓扑边界。
...
def train(args):
    ...
    logger.info(f"🚀 初始化泛癌训练任务 (Multi-Positive + AdamW + Schedulers) | Device: cuda")
    ...
    # ---------------------------------------------------------
    # 阶段一：预训练 (✨ 优化 8: AdamW + Warmup + Cosine)
    # ---------------------------------------------------------
    ...
    plt.title('Training Loss Pipeline (Multi-Positive & Schedulers)')
