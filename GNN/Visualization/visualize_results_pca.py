...
# ================= 🛡️ 架构解耦区 (锁定为评估专用) =================
# 将模型直接写在这里，防止受到 train.py 后续调参修改的影响
...
class GCLModel_Morph(nn.Module):
    def __init__(self, in_channels=233, hidden_channels=128, out_channels=32, n_clusters=20):
        super().__init__()
        # 🌟 核心修复点 1：锁定评估架构的注意力头数为 8，与权重绝对对齐
        self.heads1 = 8 
...
def plot_spatial_comparison(pt_path, expr_path, model_path, truth_path, hidden_dim, out_dim, n_clusters=20, save_name="comparison.png"):
    DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    # ================= 1. 加载模型与特征 =================
    print(f"[-] 正在从 {model_path} 加载模型权重...")
    
    # 架构已解耦，强制使用本文件内修正后的 GCLModel_Morph
    ...
    merged = pd.merge(coord_df, truth_full[['ID', x_col, y_col, 'fine_annot_type']], left_on='barcode', right_on='ID', how='inner')
    merged = merged.rename(columns={x_col: 'x', y_col: 'y', 'fine_annot_type': 'annot_type'})
...
if __name__ == "__main__":
    CONFIG = {
        ...
        "hidden_dim": 128,
        "out_dim": 32,
        # 🌟 核心修复点 2：绝不可改！必须与训练保存时的 20 类保持一致，映射交给代码去管。
        "n_clusters": 20, 
